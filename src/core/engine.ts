...
export function analyzeRoll(message: any): RollData | null {
  ...
  if (isTableRoll(roll, message)) return null;

  const actor = getActorFromMessage(message);
  const d20Die = roll.dice?.find((d: any) => d.faces === 20);

  if (actor && d20Die?.results?.length) {
    return analyzeCharacterRoll(d20Die, roll, actor, message);
  }

  if (getSetting("checkManualRolls")) {
    return analyzeManualRoll(roll, message);
  }

  return null;
}

function isTableRoll(roll: any, message: any): boolean {
  return !!(
    roll.options?.table ||
    roll._formula?.includes("RollTable") ||
    message.flags?.core?.RollTable
  );
}

function getActorFromMessage(message: any): any {
  return message.speaker?.actor
    ? game.actors?.get(message.speaker.actor)
    : null;
}

function analyzeCharacterRoll(
  d20Die: any,
  roll: any,
  actor: any,
  message: any
  ...
  const category = detectCategory(
    `${message.content || ""} ${message.flavor || ""}`
  );
...
function analyzeManualRoll(roll: any, message: any): RollData | null {
...
export function checkPermission(rollData: RollData, userId: string): boolean {
  ...
  const playerOwners = Object.keys(rollData.actor.ownership || {}).filter(
    (id) => {
      const user = game.users.find((u: any) => u.id === id);
      return (
        rollData.actor.ownership[id] === 3 &&
        id !== "default" &&
        user &&
        !user.isGM
      );
...
function detectCategory(text: string): AttackCategory | null {
  ...
  if (
    getSetting("checkAbility") &&
    ABILITY_KEYWORDS.some((k) => lower.includes(k))
  )
    return "ability";
  return null;
}
