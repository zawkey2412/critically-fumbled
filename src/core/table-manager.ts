...
export async function drawConfiguredTable(
  params: RollTableDrawParams
): Promise<void> {
  ...
  try {
    ...
    const autoRoll = getSetting(
      actor?.hasPlayerOwner ? "autoRollPlayers" : "autoRollNPCs"
    );
    ...
    if (autoRoll) {
      ...
      const resultText = result.results
        .map((r: any) => r.text || r.getChatText())
        .join(", ");
