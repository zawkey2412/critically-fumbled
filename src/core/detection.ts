...
export function mapActionTypeToCategory(
  actionType: string
): AttackCategory | null {
...
export function detectRollCategory(text: string): "save" | "ability" | null {
  ...
  if (getSetting("checkSaves") && lowerText.includes("saving throw"))
    return "save";
  if (
    getSetting("checkAbility") &&
    ABILITY_KEYWORDS.some((keyword) => lowerText.includes(keyword))
  )
    return "ability";
